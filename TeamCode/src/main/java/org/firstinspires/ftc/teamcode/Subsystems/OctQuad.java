...
public class OctQuad extends SubsystemBase {
    ...
    public OctQuad() {
        ...
        octoquad.setSingleEncoderDirection(0, OctoQuad.EncoderDirection.FORWARD);
        octoquad.setSingleEncoderDirection(1, OctoQuad.EncoderDirection.REVERSE);
        octoquad.setSingleEncoderDirection(2, OctoQuad.EncoderDirection.FORWARD);
    ...
    public void periodic() {
        ...
        OctoQuad.EncoderDataBlock encoderDataBlock = octoquad.readAllEncoderData();
        //int[] positions = octoquad.readAllPositions();
        if (encoderDataBlock.positions.length >= 3)
        {
            // go ahead and read values
            OdometryRightEncoder = encoderDataBlock.positions[0];
            OdometryLeftEncoder = encoderDataBlock.positions[1];
            OdometryFrontEncoder = encoderDataBlock.positions[2];
        }
